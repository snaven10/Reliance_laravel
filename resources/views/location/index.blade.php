  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{ $e->shelvest }}</td>
        @can($table.'.update')
        <th><a href="{{ route($table.'.edit', ['location' => $e->id ]) }}"><i class="material-icons">update</i></a></th>
        @endcan
        @can($table.'.destroy')
        <th>
          <form action="{{ route($table.'.destroy', ['location' => $e->id ]) }}" method="post" class="frmDelete">
            @csrf
            @method('DELETE')
            <button
